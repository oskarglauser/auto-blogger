export async function generateArticle(openai, topic, inventory, config, lang, research) {
    const langPrompts = config.prompts[lang];
    if (!langPrompts) {
        throw new Error(`No prompts configured for language "${lang}". Add prompts for this language in your config.`);
    }
    // Build internal link list for the prompt
    const linkTargets = inventory.linkTargets
        .filter((t) => t.lang === lang)
        .map((t) => {
        const baseUrl = t.type === 'guide'
