 ...
 */
export function validateInternalLinks(markdown, inventory, config, lang) {
    const validSlugs = new Set([...inventory.posts, ...inventory.guides]
    ...
    return markdown.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, text, url) => {
        ...
        const isGuidePath = normalizedUrl.includes(config.internalLinks.guidesBaseUrl);
        const isBlogPath = normalizedUrl.includes(config.internalLinks.blogBaseUrl);
...
//# sourceMappingURL=internal-linker.js.map