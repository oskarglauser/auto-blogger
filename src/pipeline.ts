...
import type { AutoBloggerConfig, TopicSuggestion, ContentInventory, WriteResult } from './types.js';
...
export async function runPipeline(
    ...
    if (targetLang) {
        ...
        const primaryResult = await researchAndGenerate(
            openai, inventory, config, primaryLang, selectedTopic, dryRun
        );
        ...
        for (const lang of config.languages.slice(1)) {
            ...
            await researchAndGenerate(
                openai, inventory, config, lang, translatedTopic, dryRun, primaryResult?.slug
            );
        }
    }
...
Return ONLY the JSON object, no markdown formatting.`,
    ...
    const translated = parseJsonResponse<Record<string, unknown>>(content, {});
